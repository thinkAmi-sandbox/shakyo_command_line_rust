...
pub fn get_args() -> MyResult<Config> {
    ...
    Ok(Config {
        ...
        today: NaiveDate::from(today.naive_local())
    })
}
    ...
    match parse_int(month) {
        Ok(num) => {
            if (1..=12).contains(&num) {
                ...
                Err(format!(
                    "month \"{}\" not in the range 1 through 12", month
                ).into())
            }
        }
        _ => {
...
fn last_day_in_month(year: i32, month: u32) -> NaiveDate {
    ...
    // NaiveDate::from_ymd がdeprecated なので差し替えた
...
mod tests {
    ...
    fn test_format_month() {
        // NaiveDate::from_ymd が deprecated なので差し替えた
